...
from ooijh.drops import DROP_ADCP_VARS, drop_qartod_test_vars


class ADCP(KDATA):
    def __init__(self, site: str, node: str, instrument: str = 'ADCP', stream: str = 'adcp', 
        ...
        """
        A class for obtaining preprocessed and processed ADCP data.
    ...
    def preprocess(self, ds_list: list) -> list:
        ...
        ds_list = [ds.drop_vars(DROP_ADCP_VARS, errors = 'ignore') for ds in ds_list]
    ...
    def process(self, ds: xr.Dataset) -> xr.Dataset:
        ...
        median_bins = np.median(ds.bin_depths, axis = 0) # Create a common depth bin.
        ds['median_depth'] =(['bin'], median_bins)
        ds = ds.swap_dims({'bin':'median_depth'})
        ds = ds.drop_vars(['bin'], errors = 'ignore')
        return ds
